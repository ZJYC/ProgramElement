...
size_t ringbuf_bytes_free(const struct ringbuf_t *rb){
    if (rb->head >= rb->tail)return ringbuf_capacity(rb) - (rb->head - rb->tail);
    else return rb->tail - rb->head - 1;
}
...
size_t ringbuf_findchr(const struct ringbuf_t *rb, int c, size_t offset){
    ...
    if (found)return offset + (found - start);
    else return ringbuf_findchr(rb, c, offset + n);
}
